...
TOKEN = "6753054538:AAGlbZnBbbtQHqy7nvAD08UaN8huwpJHXwk"
id = 995944815
...
def start_cmd(m):
    ...
    bot.send_message(
        u_id, "أهلاً بك عزيزي في بوت التواصل ارسل رسالتك ليتم ارسالها للمالك ."
    )
...
def admin_cmd(m):
    ...
    markup.add(
        ...
        InlineKeyboardButton(
            "🔒 تفعيل الاشتراك الإجباري", callback_data="enable_forced_sub"
        ),
        InlineKeyboardButton(
            "🔓 إيقاف الاشتراك الإجباري", callback_data="disable_forced_sub"
        ),
    ...
    bot.send_message(
        m.chat.id, "اهلا بك عزيزي المالك في لوحة التحكم الخاصه بك ", reply_markup=markup
    )
...
def callback_query(call):
    ...
    if call.data == "ban":
        msg = bot.send_message(
            call.message.chat.id, "أرسل معرف أو آيدي المستخدم لحظره:"
        )
        bot.register_next_step_handler(msg, ban_user)
    elif call.data == "unban":
        msg = bot.send_message(
            call.message.chat.id, "أرسل معرف أو آيدي المستخدم لفك حظره:"
        )
        bot.register_next_step_handler(msg, unban_user)
    elif call.data == "add_admin":
        msg = bot.send_message(
            call.message.chat.id, "أرسل معرف أو آيدي المستخدم لإضافته كأدمن:"
        )
        bot.register_next_step_handler(msg, add_admin)
    elif call.data == "remove_admin":
        msg = bot.send_message(
            call.message.chat.id, "أرسل معرف أو آيدي المستخدم لحذفه من الأدمن:"
        )
...
def remove_admin(m):
    ...
    if u.isdigit():
        ...
        try:
            ...
            bot.send_message(
                m.chat.id, "تعذر إزالة الأدمن. تحقق من صحة المعرف أو الآيدي."
            )
...
def set_forced_channel(m):
    ...
    try:
        ...
        bot.send_message(
            m.chat.id, "تعذر إضافة القناة. تأكد من أن البوت مشرف في القناة."
        )
...
def handle_message(m):
    ...
    if not communication_enabled:
        if u_id != id:
            bot.send_message(
                u_id,
                f"تم إيقاف استلام الرسائل من قبل المالك. @{bot.get_chat(id).username}",
            )
        return
    ...
    if u_id != id:
        ...
        bot.send_message(
            id, f"new message from : @{m.from_user.username}:\nmessage : {m.text}"
        )
        bot.send_message(u_id, "تم إرسال رسالتك إلى المالك انتضر الرد منه .")
    else:
        if m.reply_to_message:
            ...
            for user_id, msg_id in sessions.items():
                if m.reply_to_message.message_id == msg_id:
                    break
            if user_id:
                bot.send_message(user_id, f"{m.text}")
            else:
